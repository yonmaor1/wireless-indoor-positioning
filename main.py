...
# from blupy import Scanner <-- previously used
...
class PositionCalculator:
    def __init__(self, tech_type):
        ...
        self.xsig = 4 if tech_type == 'wifi' else 5

    def rssi_to_distance(self, rssi, tx_power):
        return 10 ** ((tx_power - rssi - self.pl0 - self.xsig) / (10 * self.n))
...
async def scan_ble():
    scanner = Scanner()
    devices = await scanner.discover(5.0)
    ble_devices = {}
    ...
    try:
        while True:
            ...
            if len(wifi_measurements) >= 3:
                ...
                print(f"{datetime.now().isoformat()} - {wifi_measurements}\n")
                wifi_log_file.write(f"{datetime.now().isoformat()} - {wifi_measurements}\n")
                # wifi_log_file.write(f"{datetime.now().isoformat()} - {wifi_pos[0]:.2f} - {wifi_pos[1]:.2f}\n")
            
            # BLE Positioning
            ble_measurements = await scan_ble()
            if len(ble_measurements) >= 3:
                ...
                print(f"{datetime.now().isoformat()} - {ble_measurements}\n")
                ble_log_file.write(f"{datetime.now().isoformat()} - {ble_measurements}\n")
